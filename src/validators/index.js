...
export const projectSchema = z.object({
  ...
  status: z.enum(["TODO", "PROGRESS", "ON_HOLD", "FINISHED"], {
    errorMap: () => ({ message: "Invalid status" }),
  }).optional(),
  isSelected: z.string().optional().transform(val => val === "true"),
  isHidden: z.string().optional().transform(val => val === "true"),
  stacks: z.string().optional().transform(val => (val ? val.split(",") : [])),
}).passthrough();
...
export const skillSchema = z.object({
  ...
  order: z.string().regex(/^\d+$/, "Order must be a number").transform(Number).optional(),
}).passthrough();

export const signInSchema = z.object({
  email: z.string().min(1, "Enter your email address").email("Invalid email format"),
...
}).passthrough().refine(data => data.newPassword === data.newPassword2, {
